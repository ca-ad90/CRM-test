...
export const usersDb = {
  ...
  async getAll() {
    ...
    return db.all(
      "SELECT user_id, username, email, created_at, last_login, is_admin FROM users"
    );
  },
  ...
  async getById(id) {
    const db = await getDbConnection();
    return db.get(
      "SELECT user_id, username, email, created_at, last_login, is_admin FROM users WHERE user_id = ?",
      id
    );
  },
  ...
  /**
   * Create a new user
   * @param {object} userData - The user data (username, email, password, is_admin)
  ...
  async create(userData) {
    ...
    const isAdmin = userData.is_admin ? 1 : 0;

    const result = await db.run(
      `INSERT INTO users (username, email, password_hash, is_admin) VALUES (?, ?, ?, ?)`,
      [userData.username, userData.email, passwordHash, isAdmin]
    );
   ...
   * @param {number} id - The user ID
   * @param {boolean} isAdmin - The admin status
   * @returns {Promise<boolean>} Success status
   */
